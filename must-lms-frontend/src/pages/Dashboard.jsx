...
const Dashboard = () => {
    ...
    const [dashData, setDashData] = useState({ fullName: '', gpa: 0, courseCount: 0, pendingTasks: 0 });
    ...
    useEffect(() => {
        ...
        // اختيار جملة عشوائية عند أول رندر فقط
            ...
            <div className="max-w-7xl mx-auto space-y-10 pb-20">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 dark:border-white/10 pb-10">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-2 font-bold italic uppercase text-xs tracking-widest">Faculty of Engineering • Computer Science Dept.</p>
                    </motion.div>
                    ...
                    <div className="relative group">
                        ...
                        <div className="glass-card relative px-8 py-5 flex items-center gap-5 bg-white/50 dark:bg-slate-900/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[
                        { label: 'Registered Courses', val: String(dashData.courseCount).padStart(2, '0'), icon: <BookOpen />, color: 'text-blue-500', bg: 'bg-blue-500/10', path: '/student/courses' },
                        { label: 'Current GPA', val: dashData.gpa?.toFixed(2) || '0.00', icon: <Award />, color: 'text-[#a78bfa]', bg: 'bg-[#a78bfa]/10', path: '/student/grades' },
                        { label: 'Pending Tasks', val: String(dashData.pendingTasks).padStart(2, '0'), icon: <ClipboardList />, color: 'text-cyan-500', bg: 'bg-cyan-500/10', path: '/student/assignments' }
                    ].map((stat, i) => (
                        <motion.div
                            key={i}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                        ...
                        >
                            ...
                            <div className="relative z-10 space-y-6">
                                <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic">
                                    Welcome back, <br />
                                    <span className="text-blue-600 dark:text-[#a78bfa]">{dashData.fullName || 'Future Engineer'}</span>
                                </h2>
                                ...
                                <p className="text-slate-500 dark:text-slate-400 font-bold max-w-lg leading-relaxed text-sm">
                                    Your academic journey is on track. Focus on your goals today.
                                </p>
                                <div className="flex flex-wrap gap-4 pt-4">
                                    <Link to="/student/courses" className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-950 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="glass-card p-8 space-y-6 border border-slate-100 dark:border-white/5 bg-white/70 dark:bg-slate-900/60">
                            {[
                                { title: 'Signals Assignment', time: 'Today, 11:59 PM', icon: <Bell />, color: 'text-red-400', bg: 'bg-red-400/10' },
                                { title: 'Control Systems Quiz', time: 'Tomorrow, 10:00 AM', icon: <ClipboardList />, color: 'text-amber-500', bg: 'bg-amber-500/10' }
                            ].map((note, idx) => (
