...
import {
    ...
    Zap, Target, Heart, Edit2
} from 'lucide-react';

const Profile = () => {
    // 💡 بيانات الطالب (مطابقة للـ SQL Schema والـ Login)
    const userData = {
        FullName: "Eng. Abdelrahman Noaman",
        Email: "Abody.cs@university.edu",
    ...
    const progressOffset = circumference - (userData.GPA / 4.0) * circumference;

    // أنيميشن لدخول العناصر
    ...
    return (
        ...
        >
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                {/* 1. New Elegant Header (بدون كفر ضخم) */}
                        ...
                        <div className="relative group">
                            ...
                            <div className="relative w-40 h-40 rounded-full bg-white dark:bg-slate-900 p-2 shadow-xl ring-1 ring-slate-200/50 dark:ring-white/5">
                                <div className="w-full h-full rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden">
                                    <User size={70} className="text-slate-300 dark:text-slate-600" />
                                </div>
                            </div>
                        ...
                        <div className="space-y-2.5 pb-2">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start items-center gap-3">
                                <span className="flex items-center gap-2 text-xs font-black text-[#a78bfa] uppercase tracking-widest bg-[#a78bfa]/10 px-4 py-1.5 rounded-full border border-[#a78bfa]/20 shadow-inner">
                                    <Zap size={14} fill="currentColor" /> {userData.Level}
                                </span>
                ...
                {/* 2. New Stats & Info Grid (توزيعة مختلفة) */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">

                    {/* GPA & Quick Stats - Left Column */}
                    <motion.div variants={itemVariants} className="lg:col-span-5 flex flex-col gap-10">
                        {/* GPA Circle Card بلون جديد ناعم */}
                        <div className="glass-card p-10 flex flex-col items-center justify-center shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-white/5 relative bg-white/70 dark:bg-slate-900/60">
                            ...
                            <div className="relative">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-6xl font-black text-slate-950 dark:text-white italic tracking-tighter leading-none">{userData.GPA}</span>
                    ...
                    <motion.div variants={itemVariants} className="lg:col-span-7 flex flex-col gap-6 h-full">
                        <div className="glass-card p-10 flex-1 border border-slate-100 dark:border-white/5 bg-white/70 dark:bg-slate-900/60">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-12">
                                ...
                                <ModernInfoItem icon={<GraduationCap size={20} />} label="Current level" value={userData.Level} />
                                <ModernInfoItem icon={<Calendar size={20} />} label="Registration" value={userData.JoinDate} />
                                <ModernInfoItem icon={<Award size={20} />} label="Earned units" value={`${userData.TotalCredits} Credits`} />
                            </div>
                        </div>
                    </motion.div>
                </div>
...
// Component داخلي بلون جديد (Pastel)
