 ...
 * This file defines the routing structure, layout, and global state wrappers for the entire frontend.
 */
 ...
 * Automatically redirects the user to their role-specific dashboard (Student, Instructor, or Assistant)
 * when they visit the generic /dashboard path.
 */
 ...
 * Wraps routes that require authentication. If no user is logged in, redirects to /login.
 * Also handles role-based access control if allowedRoles is provided.
 */
...
/**
 * SocketWrapper Component
 * Defines the primary layout for authenticated users, including the Sidebar and the Main content area.
 * It also initializes the SocketProvider to enable real-time updates for logged-in users.
 */
...
const AppContent = () => {
    ...
    // Dynamically get user ID for persistent socket connection
    const userString = localStorage.getItem('user');
    let userId = null;
    if (userString && userString !== 'undefined') {
        try {
